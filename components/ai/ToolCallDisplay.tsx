...
const ToolCallDisplay: React.FC<{ toolCalls: ToolCall[] }> = ({ toolCalls }) => {
    ...
    // Determine the overall color based on status priority: error > in_progress > success
    ...
    const headerColor = hasError ? 'text-red-400' : hasInProgress ? 'text-vibe-accent' : 'text-green-400';
    const borderColor = hasError ? 'border-red-400/30' : hasInProgress ? 'border-vibe-accent/30' : 'border-green-400/30';
    const bgColor = hasError ? 'bg-red-900/20' : hasInProgress ? 'bg-vibe-accent/10' : 'bg-green-900/20';
