...
import { Project, GitCredential } from '../../types';
import PlusIcon from '../icons/PlusIcon';
import TrashIcon from '../icons/TrashIcon';
import CogIcon from '../icons/CogIcon';
import SpinnerIcon from '../icons/SpinnerIcon';
import GitCredentialSelectionModal from './GitCredentialSelectionModal';
...
interface ProjectManagementModalProps {
  ...
  onCloneProject: (url: string, name: string, credentialId?: string | null) => Promise<void>;
  isCloning?: boolean;
...
const ProjectManagementModal: React.FC<ProjectManagementModalProps> = (props) => {
  const { 
      isOpen, onClose, projects, activeProject, onNewProject, onSwitchProject, 
      onDeleteProject, onOpenProjectSettings, onCloneProject, isCloning, 
      cloningProgress, gitCredentials, onManageCredentials 
  } = props;
  
  const [newProjectName, setNewProjectName] = useState('');
  const [cloneUrl, setCloneUrl] = useState('');
  const [cloneProjectName, setCloneProjectName] = useState('');
  const [selectedCloneCredentialId, setSelectedCloneCredentialId] = useState<string | null>(null);
  const [isCredentialSelectorOpen, setIsCredentialSelectorOpen] = useState(false);
  ...
  const handleClone = async () => {
      if (cloneUrl.trim() && cloneProjectName.trim()) {
          await onCloneProject(cloneUrl, cloneProjectName, selectedCloneCredentialId);
      }
  }

  const selectedCredential = gitCredentials.find(c => c.id === selectedCloneCredentialId);
  const defaultCredential = gitCredentials.find(c => c.isDefault);
  let credentialButtonText = 'Auth: Default';
  if (selectedCredential) {
      credentialButtonText = `Auth: ${selectedCredential.name}`;
  } else if (!defaultCredential) {
      credentialButtonText = 'Auth: None';
  }
  ...
  return (
    <>
          ...
          <div className="overflow-y-auto p-4 space-y-6">
            ...
            <div className="border-t border-vibe-bg-deep pt-4">
              ...
              <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                      <button 
                          onClick={() => setIsCredentialSelectorOpen(true)}
                          className="flex-shrink-0 bg-vibe-bg text-vibe-text-secondary px-3 py-2 rounded-md text-sm hover:bg-vibe-comment"
                          title="Select authentication credential"
                      >
      ...
      <GitCredentialSelectionModal
        isOpen={isCredentialSelectorOpen}
        onClose={() => setIsCredentialSelectorOpen(false)}
        credentials={gitCredentials}
        onSelect={(id) => {
            setSelectedCloneCredentialId(id);
            setIsCredentialSelectorOpen(false);
        }}
        onClearSelection={() => {
            setSelectedCloneCredentialId(null);
            setIsCredentialSelectorOpen(false);
        }}
        onManageCredentials={() => {
            setIsCredentialSelectorOpen(false);
