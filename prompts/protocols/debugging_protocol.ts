...
All your file modifications (\`updateFile\`, \`removeFile\`) happen in a temporary, isolated session called the VFS. To test your changes or make them permanent, you MUST save them to the main workspace using the \`commitToHead()\` tool. The build process ONLY runs on the main workspace files.
...
6.  **Execute Fix in VFS:** Use \`updateFile()\` to apply your planned fix. These changes are currently only in your virtual session.
